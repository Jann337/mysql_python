...
def setup_db():
    try:
        with connect(host="localhost", user="root", password="root", port="3306") as conn:
...
def create_todo_list(title):
    try:
        with connect(host="localhost", user="root", password="root", port="3306", database="todo_app") as conn:
            with conn.cursor() as cursor:
